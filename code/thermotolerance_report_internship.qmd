...
DURIN_Methods <- DURIN_data %>% 
  dplyr::select(species, plant_nr, TempExpTime, TempExp, FvFm_Final_num, PlantSampleID)
...
psiiht(Temperature=DURIN_Methods$TempExp, FvFm=DURIN_Methods$FvFm_Final_num, control.temp=20, id=DURIN_Methods$species, plant_nr=DURIN_Methods$plant_nr, TempExpTime=DURIN_Methods$TempExpTime, plot.est=T, boots=9) 

T50metrics <- psiiht(Temperature=DURIN_Methods$TempExp, FvFm=DURIN_Methods$FvFm_Final_num, control.temp=20, id=DURIN_Methods$species, plant_nr=DURIN_Methods$plant_nr, TempExpTime=DURIN_Methods$TempExpTime, plot.est=T, boots=9) 

P3_metrics <- dplyr::filter(T50metrics, plant_nr == "3")



## Export the T50 metrics data
write.csv(t50metrics, file = 'C:/Users/albat/Dropbox/DURIN/stats/DURIN_TT_Testing/T50_metrics_lowboots.csv', row.names = FALSE)
```
...
T50_clean <- T50_new_metrics %>% 
  dplyr::select(id, plant_nr, T50.mn) %>% 
  dplyr::rename(c("TimeTreatment"="id", "PlantNr"="plant_nr", "T50"="T50.mn" )) %>%   na.omit()
...
(time_boxplot <- ggplot(T50_clean, aes(x = TimeTreatment, y = T50,
                                            fill = TimeTreatment)) +
                 geom_boxplot() +
...
T50.lmer <- lmer(T50 ~ TempExpTime + (1|PlantNr), data = T50_clean)
...
T50.emm2 <- emmeans(T50.lmer, "TempExpTime")
T50.emm2
...
setwd("C:/Users/albat/Dropbox/DURIN/stats/DURIN_TT_Testing/")
library(car)
...
DURIN_Methods <- DURIN_data %>% 
  dplyr::select(species, plant_nr, TempExpTime, TempExp, FvFm_Final_num, PlantSample)

DURIN_Methods$PlantSample = paste("P", DURIN_Methods$plant_nr, "_T", DURIN_Methods$TempExpTime)

## Run the function


psiiht(Temperature=DURIN_Methods$TempExp, FvFm=DURIN_Methods$FvFm_Final_num, control.temp=20, id=DURIN_Methods$TempExpTime, plant_nr=DURIN_Methods$plant_nr, plot.est=T, boots=5) ## considering plant_nr

psiiht(Temperature=DURIN_Methods$TempExp, FvFm=DURIN_Methods$FvFm_Final_num, control.temp=20, id=DURIN_Methods$TempExpTime, plot.est=T, boots=5) ## not considering plant_nr, just one mean value per Time Treatment

T50_new_metrics <- psiiht(Temperature=DURIN_Methods$TempExp, FvFm=DURIN_Methods$FvFm_Final_num, control.temp=20, id=DURIN_Methods$TempExpTime, plot.est=T, boots=5)
```
...
## Clean the dataset (select just the T50)
T50_clean <- T50_new_metrics %>% 
  dplyr::select(id, T50.mn) %>% 
  dplyr::rename(c("TimeTreatment"="id", "T50"="T50.mn" )) %>%   
  na.omit()

## Makes TempExpTime into factor variable 
T50_clean$TimeTreatment <- as.factor(as.character(T50_clean$TimeTreatment))   

 

## Reordering a factor's levels

levels(T50_clean$TimeTreatment)  # shows the different factor levels in their default order

T50_clean$TimeTreatment <- factor(T50_clean$TimeTreatment,
                                 levels = c('1', '5', '10', '15','20'),   # whichever order you choose will be reflected in plots etc
                                 labels = c('1', '5', '10', '15','20')    # Make sure you match the new names to the original levels!
                                 ) 
...
T50.lm <- lm(T50 ~ TimeTreatment, data = T50_clean)
T50.lm

T50.emm <- emmeans(T50.lm, "T50")
T50.emm
